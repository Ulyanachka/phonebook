def work_with_phonebook(phonebook):
    while True:
        ...
        choice = input('1 - Импорт файла\n2 - Найти абонента\n3 - Добавить абонента\n\
4 - Изменить контакт\n5 - Удалить контакт\n6 - Отобразить весь справочник\n0 - Выйти из приложения\n')
        print()
        if choice == '1':
            ...
            print('Ошибка, убедитесь что ввели всё правильно')
            print()
            continue


def import_data(file_to_add, phonebook): #импорт
    try:
...
def find_number(contact_list): #найти контакт
...
def add_phone_number(file_name): #добавить
    info = ' '.join(get_new_number())
    with open(file_name, 'a', encoding='utf-8') as file:
        file.write(f'{info}\n')


def show_phonebook(file_name): #список
...
def search_to_modify(contact_list: list): #несколько контактов
    search_field, search_value = search_parameters()
    search_result = []
    for contact in contact_list:
